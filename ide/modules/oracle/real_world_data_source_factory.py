...
class RealWorldDataSourceFactory():
    ...
    def NASDAQ(self):
        filename = '/real_world_data_sets/NASDAQ/WIKI-PRICES.csv'
        df = pd.read_csv(filename)
        q = list(range(1, len(df.index)))
        return RealWorldDataSetDataSource(q,df.to_numpy())
    
    def smartphone(self):
        df = pd.read_csv('/real_world_data_source/Smartphone/final_acc_test.tx', sep=" ", header=None)
    ...
    def hydraulic(self):
        df = pd.read_csv('/real_world_data_source/HydraulicSystemsDataSet/PS1', sep=" ", header=None)
        df2 = pd.read_csv('/real_world_data_source/HydraulicSystemsDataSet/PS2', sep=" ", header=None)
        df3 = pd.read_csv('/real_world_data_source/HydraulicSystemsDataSet/PS3', sep=" ", header=None)
        df4 = pd.read_csv('/real_world_data_source/HydraulicSystemsDataSet/PS4', sep=" ", header=None)
        df.merge(df2,on = df.columns.values.tolist(), how = 'right')
        df.merge(df3,on = df.columns.values.tolist(), how = 'right')
        df.merge(df4,on = df.columns.values.tolist(), how = 'right')
        q = list(range(1, len(df.index)))
